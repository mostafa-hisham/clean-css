...
function font(property, configuration, validator) {
  ...
  if (Object.prototype.hasOwnProperty.call(property, 'all') && (values.length < 2 || !_anyIsFontSize(values, validator) || !_anyIsFontFamily(values, validator))) {
...
function widthStyleColor(property, configuration, validator) {
  ...
  var values = property.value.slice(0);
  var hashArr = [];
  var allValStr= values.join('');
  if (values.length > 0 && values.length < 3 && allValStr.includes('#')) {
    for (var indx = 0; indx < values.length; indx++) {
      if (!values[indx].changed && values[indx][1].includes('#')) {
        var arr = values[indx][1].split(/(?=#)/g);
        hashArr = [...hashArr, ...arr];
      } else {
        hashArr.push(values[indx][1]);
      }
    }
  }
  if (hashArr.length > 0) {
