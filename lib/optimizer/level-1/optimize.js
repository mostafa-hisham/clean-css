...
function optimizeBody(rule, properties, context) {
  ...
  var valueOptimizers;
  var property, name, type, value, tempValue;
  var propertyToken;
  ...
  for (var i = 0, l = tokens.length; i < l; i++) {
    ...
    case Token.AT_RULE:
      ...
      /* if (Array.isArray(token[1])
            && token[1].length > 0
            && (token[1][0][1].toLowerCase().includes('@charset')
              || token[1][0][1].toLowerCase().includes('@import'))
      ) {
        token[1] = token[1][0][1];
      } */
