...
function better_normalize(value) {
    return value
        .replace(/["']+/g, '')
        .replace(importantRegex, '');
}
  ...
  if (token[0] == Token.AT_RULE_BLOCK && token[1][0][1] == '@font-face') {
    for (i = 0, l = token[2].length; i < l; i++) {
      ...
      if (property[1][1] == 'font-family') {
        match = better_normalize(property[2][1].toLowerCase());
        atRules[match] = atRules[match] || [];
        atRules[match].push(token);
        break;
      }
    ...
    for (i = 0, l = token[2].length; i < l; i++) {
      ...
      if (property[1][1] == 'font') {
        ...
        for (j = 0, m = component.value.length; j < m; j++) {
          normalizedMatch = better_normalize(component.value[j][1].toLowerCase());
      ...
      if (property[1][1] == 'font-family' || property[1][1].startsWith('--')) {
        for (j = 2, m = property.length; j < m; j++) {
          normalizedMatch = better_normalize(property[j][1].toLowerCase());
